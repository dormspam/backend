...
def abs_date_from_phrase(text):
    ...
    for month in months:
        ...
        regex = r'\b(' + month + \
            r'.? |((0?[1-9]|10|11|12)[/]))(the )?' + \
            day_range + r'(\b|st|nd|rd|th)'
...
def get_date(match):
    ...
    """
    text = match.group()
    split = [word for word in re.split(
        r'[ ./-]+', text) if word.lower() != 'the']
    raw_month = split[0]
...
def start_end_time_from_phrase(text):
    ...
    startend_re = r'(\b([0-9]|10|11|12) ?([-–]|to) ?(' + time_re + \
        r'))|((' + time_re + r') ?([-–]|to) ?(' + time_re + r'))'
